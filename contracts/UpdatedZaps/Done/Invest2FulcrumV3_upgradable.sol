...
contract Invest2FulcrumV3_upgrabable_ERC is Initializable, ReentrancyGuard {
    ...
    function initialize(IKyberInterface _KyberInterfaceAddress, uint16 _goodwillInBasisPoints, address _dzGoodwillAddress) initializer public {
        ReentrancyGuard.initialize();
        stopped = false;
        owner = msg.sender;
        goodwillInBasisPoints = _goodwillInBasisPoints;
        dzGoodwillAddress = _dzGoodwillAddress;
        KyberInterfaceAddress = _KyberInterfaceAddress;
        maxPrice = 0;
    }
    
    // to change the goodwill basis points
    function set_goodwillInBasisPoints(uint16 _new_goodwillInBasisPoints) public onlyOwner {
        require(_new_goodwillInBasisPoints < 10000 && _new_goodwillInBasisPoints > 0, "Not correct goodwill rate in basis Points");
        goodwillInBasisPoints = _new_goodwillInBasisPoints;
    }

    // to change the dzGoodwillAddress 
    function set_dzGoodwillAddress(address _new_dzGoodwillAddress) public onlyOwner {
        dzGoodwillAddress = _new_dzGoodwillAddress;
    }
    
    // to change the KyberInterfaceAddress
    function set_KyberInterfaceAddress(address _new_KyberInterfaceAddress) public onlyOwner {
        KyberInterfaceAddress = IKyberInterface(_new_KyberInterfaceAddress);
    }
    ...
    function LetsInvest(address _FuclrumOnwardAddress, address _destTokenAddress, uint _slippage, address _towhomtoissue) payable public returns (uint) {
        ...
        uint remaining_tokens = SafeMath.sub(_destTokens,dzportion);
        IERC20(_destTokenAddress).approve(_FuclrumOnwardAddress, remaining_tokens);
        IfulcrumInterface FA = IfulcrumInterface(_FuclrumOnwardAddress);
        uint FulcrumTokens = FA.mintWithToken(_towhomtoissue, _destTokenAddress, remaining_tokens, maxPrice);
