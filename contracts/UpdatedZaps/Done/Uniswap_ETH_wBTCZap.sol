...
contract UniSwap_ETH_wBTCZap is Initializable {
    ...
    address public WBTC_TOKEN_ADDRESS;
    ...
    function initialize (address _UniSwapAddLiquityV2_GeneralAddress, address _WBTC_TOKEN_ADDRESS) initializer public {
        stopped = false;
        owner = msg.sender;
        UniSwapAddLiquityV2_GeneralAddress = UniSwapAddLiquityV2_General(_UniSwapAddLiquityV2_GeneralAddress);
        WBTC_TOKEN_ADDRESS = _WBTC_TOKEN_ADDRESS;
    }
    ...
    // this function should be called should we ever want to change the wBTC token address
    function set_new_WBTC_TOKEN_ADDRESS(address _new_WBTC_TOKEN_ADDRESS) public onlyOwner {
        WBTC_TOKEN_ADDRESS = _new_WBTC_TOKEN_ADDRESS;
    }
    ...
    function LetsInvest(address payable _towhomtoIssueAddress, uint _slippage, bool _residualInToken) public payable returns(uint) {
        uint LiquidityTokens = UniSwapAddLiquityV2_GeneralAddress.LetsInvest.value(msg.value)(WBTC_TOKEN_ADDRESS, _towhomtoIssueAddress);
