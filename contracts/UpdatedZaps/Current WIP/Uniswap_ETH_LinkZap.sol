...
contract UniSwap_ETH_LinkZap is Initializable {
    ...
    // this function should be called should we ever want to change the LINK token address
    function set_new_LINK_TOKEN_ADDRESS(address _new_LINK_TOKEN_ADDRESS) public onlyOwner {
        LINK_TOKEN_ADDRESS = _new_LINK_TOKEN_ADDRESS;
    }
    ...
    function LetsInvest(address payable _towhomtoIssueAddress, uint _slippage, bool _residualInToken) public payable returns(uint) {
        uint LiquidityTokens = UniSwapAddLiquityV2_GeneralAddress.LetsInvest.value(msg.value)(LINK_TOKEN_ADDRESS, _towhomtoIssueAddress);
