...
// through this contract we are putting 34% allocation to 2xLong wBTC and 66% to Uniswap pool
contract wBTC_dlWBTC2x_LLPZap is Initializable {
    ...
    (address _LLP_ERC20Token_GeneralAddress,
     ...
     address _dLWBTC2xFulCrumAddress)
    initializer public {
        stopped = false;
        owner = msg.sender;
    ...
    // this function should be called should we ever want to change the underlying Fulcrum 2xLong wBTC Contract address
    function set_wBTCFulCrumAddress(address _new_dLWBTC2xFulCrumAddress) public onlyOwner {
        dLWBTC2xFulCrumAddress = _new_dLWBTC2xFulCrumAddress;
    }
    ...
    function LetsInvest(address payable _towhomtoIssueAddress, uint _FulcrumAllocation) public payable returns(uint) {
       LLP_ERC20Token_GeneralAddress.LetsInvest.value(msg.value)(_towhomtoIssueAddress, _FulcrumAllocation, wBTCAddress, dLWBTC2xFulCrumAddress, wBTCAddress);
    }
