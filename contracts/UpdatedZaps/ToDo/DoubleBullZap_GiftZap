...
contract DoubleBullZap is Ownable {
    ...
    function LetsInvest(address _gifteeAddress) stopInEmergency public payable returns(uint) {
        uint BTC2xLongPortion = SafeMath.div(SafeMath.mul(msg.value,BTC2xLongPercentage),100);
        uint ETH2xLongPortion = SafeMath.sub(msg.value, BTC2xLongPortion);
        require (SafeMath.sub(msg.value, SafeMath.add(BTC2xLongPortion, ETH2xLongPortion)) ==0 );
        Invest2Fulcrum2xLong_BTCContract.LetsInvest2Fulcrum2xLongBTC.value(BTC2xLongPortion)(_gifteeAddress);
        Invest2Fulcrum2xLong_ETHContract.LetsInvest2Fulcrum.value(ETH2xLongPortion)(_gifteeAddress);
    }
