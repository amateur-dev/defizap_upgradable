...
contract sETH_LLP_2xETH is Initializable {
    ...
    function initialize
    (address _LLP_GeneralAddress, address _sETHAddress)
    initializer public {
        stopped = false;
        owner = msg.sender;
    ...
    // this function should be called should we ever want to change the underlying sETH Contract address
    function set_sETHAddress(address _new_sETHAddress) public onlyOwner {
        sETHAddress = _new_sETHAddress;
    }
    ...
    function LetsInvest(address payable _towhomtoIssueAddress, uint _2XLongETHAllocation, uint _slippage, bool _residualInToken) public payable returns(uint) {
       LLP_GeneralAddress.LetsInvest.value(msg.value)(_towhomtoIssueAddress, 34, sETHAddress, 5, false);
    }
