...
contract Uni_Curve_Pipe is ReentrancyGuard, Ownable {
    ...
    function Uni2Curve (
        ...
        uint256 goodwillPortion = SafeMath.div(
            SafeMath.mul(_IncomingLPT, goodwill),
            10000
        );

        require(
            IERC20(uniswapFactory.getExchange(_incomingUniUnderlyingTokenAddress)).transferFrom(
              msg.sender,
              dzgoodwillAddress,
              goodwillPortion
            ),
        ...
        require(
            IERC20(uniswapFactory.getExchange(_incomingUniUnderlyingTokenAddress)).transferFrom(
                msg.sender,
                address(this),
                SafeMath.sub(_IncomingLPT, goodwillPortion)
            ),
            "Error transferring CRV"
        );
        
        
        IERC20(uniswapFactory.getExchange(_incomingUniUnderlyingTokenAddress)).approve(
            address(uniswapRemoveLiq),
            SafeMath.sub(_IncomingLPT, goodwillPortion)
        );
        // TODO: UniSwap ZapOut to ETH
