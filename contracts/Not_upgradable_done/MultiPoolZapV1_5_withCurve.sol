...
contract MultiPoolZapV1_5 is Ownable {
    ...
    constructor(uint16 _goodwill, address payable _dzgoodwillAddress, address _curvePoolZapInAddress) public {
        goodwill = _goodwill;
    ...
    function set_curvePoolZapInAddress(address _curvePoolZapIn) onlyOwner public {
    ...
    */
    function multipleZapIn (
        ...
        uint256[] memory respectiveWeightedValues
    ) public payable {

        uint totalWeights;
        require(underlyingTokenAddresses.length + _curvePoolAddresses.length == respectiveWeightedValues.length, "Input array lengths incorrect");
        for(uint i=0; i<respectiveWeightedValues.length; i++) {
        ...
        uint uniswapPoolsCount = underlyingTokenAddresses.length;
        for(uint i=0; i<_curvePoolAddresses.length; i++) {
            CurvePoolZapIn.ZapIn.value(
                (((totalInvestable).mul(respectiveWeightedValues[i + uniswapPoolsCount])).div(totalWeights)+residualETH)
